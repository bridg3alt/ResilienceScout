...
def main():
    ...
    ok &= check("indoor swing damped vs outdoor swing",
                (free["indoor_temp"].max() - free["indoor_temp"].min())
                <= (day["temp"].max() - day["temp"].min()) + 0.5,
                f"indoor swing {in_peak-free['indoor_temp'].min():.1f} vs "
                f"outdoor {out_peak-day['temp'].min():.1f}")

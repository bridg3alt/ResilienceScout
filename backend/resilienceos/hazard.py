...
class HeatwaveResult:
    ...
    peak_outdoor: float
    safe_occupancy_hours: int
    occupied_hours: int
    hours_above_caution: int
    hours_above_danger: int
    exceedance_degh: float
    profile: list
...
def backup_duration_h(b: Building, day: pd.DataFrame, start_hour: int) -> float:
    ...
    for i in range(start_hour, len(rows)):
        ...
        net = solar - load
        if net >= 0:
            stored = min(stored + net, b.battery_kwh)
            hours += 1
        else:
            deficit = -net
            if stored >= deficit:
